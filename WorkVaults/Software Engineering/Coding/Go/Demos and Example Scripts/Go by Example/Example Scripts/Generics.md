...
/* When invoking generic functions, we can often rely on _type inference_. Note that we don’t have to specify the types for `K` and `V` when calling `MapKeys` - the compiler infers them automatically.*/
    fmt.Println("keys:", MapKeys(m))

    _ = MapKeys[int, string](m) // …though we could also specify them explicitly.
